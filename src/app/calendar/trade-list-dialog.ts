...
@Component({
  ...
  template: `
    <dialog
      #dialog
      class="m-auto w-full max-w-2xl rounded-xl border border-gray-200 bg-white p-0 shadow-xl backdrop:bg-black/40"
          ...
          <tbody>
            @for (trade of summary().trades; track $index) {
              <tr class="border-b border-gray-100">
                ...
                <td class="py-2 text-right font-semibold" [class]="trade.netPL >= 0 ? 'text-green-700' : 'text-red-700'">
...
export class TradeListDialog {
  ...
  protected readonly totalPlClass = computed(() =>
    this.summary().netPL >= 0 ? 'text-green-700' : 'text-red-700',
  );
